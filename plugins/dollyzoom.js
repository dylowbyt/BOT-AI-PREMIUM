...
function getArgsText(text) {
  return text.replace(/^\.(dollyzoom|dolly|dolbyzoom|dronezoom|vertigo|vertigozoom)\s*/i, "").trim()
}
...
function buildDollyZoomPrompt(userPrompt, hasImageReference) {
  const scene = userPrompt || "subjek utama berdiri di lokasi dramatis, kamera drone dari atas turun perlahan lalu masuk efek dolly zoom yang sinematik"

  return `
Create a professional Dolly Zoom cinematic video with premium 4K film quality.

Main scene:
${scene}

Camera direction:
- Begin with a cinematic drone view / aerial establishing shot from above.
- The camera moves smoothly like a professional drone shot: slow descending movement, graceful aerial reveal, stable flight, no jitter.
- Transition from drone view into a controlled Dolly Zoom / Vertigo effect focused on the main subject.
- During the Dolly Zoom, keep the subject visually emphasized while the background perspective expands or compresses dramatically.
- The Dolly Zoom effect must be smooth, premium, realistic, and cinematic, not glitchy or exaggerated.
- Use stable camera movement, professional lens behavior, realistic motion blur, and clean cinematic framing.
- Camera movement should feel like a high-end movie trailer, luxury commercial, music video, or professional film sequence.

Visual quality:
- Crystal clear 4K look, sharp detail, clean texture, high dynamic range, premium film color grading.
- Realistic lighting, controlled shadows, atmospheric depth, cinematic contrast, polished composition.
- Smooth subject motion, stable anatomy, no flicker, no warped face, no duplicated limbs, no extra fingers.
- Avoid low resolution, blur, noise, artifacts, unstable frames, overexposure, distorted perspective, or broken anatomy.

${hasImageReference ? `Image reference rules:
- Use the uploaded image as the main visual reference or subject.
- Preserve the subject identity, face, outfit, body shape, object shape, colors, and important visual details from the image.
- Animate the image with drone view and Dolly Zoom camera movement while keeping the subject consistent.` : ""}

Technical style:
- drone view
- aerial cinematic opening
- smooth descending camera movement
- Dolly Zoom effect
- Vertigo zoom effect
- professional cinematic 4K
- realistic film lighting
- stable camera
...
async function createTask({ model, prompt, imageUrl }) {
  ...
  const input = {
    ...
    quality: "4k",
    resolution: "4k",
    camera_motion: "drone view smooth descending camera with dolly zoom vertigo effect",
    style: "professional dolly zoom cinematic 4k realistic film"
  }
  ...
  if (res.data?.code && res.data.code !== 200) {
    throw new Error(res.data?.message || "Gagal membuat task Dolly Zoom video")
  }
...
async function waitForVideo(taskId) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (state === "fail" || state === "failed" || state === "error") {
      throw new Error(data.error || data.message || "Ruxa AI gagal membuat video Dolly Zoom")
    }
  }
...
module.exports = {
  name: "dollyzoom",
  alias: ["dolly", "dolbyzoom", "dronezoom", "vertigo", "vertigozoom"],
    ...
    try {
      ...
      if (!userPrompt && !imageMessage) {
        return sock.sendMessage(from, {
          text:
            `🎥 *DOLLY ZOOM AI 4K*\n\n` +
            `Plugin khusus efek Drone View + Dolly Zoom / Vertigo Zoom.\n\n` +
            `Cara pakai teks:\n` +
            `*.dollyzoom <deskripsi video>*\n\n` +
      ...
      await sock.sendMessage(from, {
        text:
          `🎥 *Membuat Dolly Zoom video...*\n\n` +
          `🤖 Model: *${model}*\n` +
          `🚁 Kamera: drone view\n` +
          `🎬 Efek: dolly zoom / vertigo zoom\n` +
      ...
      if (imageMessage) {
        ...
        imageUrl = await uploadTmpFile(image.buffer, image.mimetype, "dollyzoom-reference.jpg")
      }

      const finalPrompt = buildDollyZoomPrompt(userPrompt, !!imageUrl)
      ...
      await sock.sendMessage(from, {
        ...
        caption:
          ...
          `📝 Prompt: ${userPrompt || "Drone view with Dolly Zoom cinematic effect"}`
      })

    } catch (err) {
      if (tokenDeducted) addTokens(sender, TOKEN_COST)
      console.log("DOLLYZOOM ERROR:", err?.response?.data || err?.message)
      await sock.sendMessage(from, {
        text:
          `❌ Gagal membuat Dolly Zoom video.\n\n` +
          (tokenDeducted ? `🪙 Token *${TOKEN_COST}* sudah dikembalikan.\n\n` : "") +
          `Error: ${translateError(err)}\n\n` +
          `Tips:\n` +
          `• Coba model lain: --model veo31 / veo3 / sora\n` +
          `• Pakai prompt jelas: subjek, lokasi, arah drone, momen dolly zoom\n` +
          `• Jika pakai foto, reply foto dengan command .dollyzoom`
      })
    }
  }
}
